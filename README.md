...
1. Open [Google Drive](https://drive.google.com) →
   **Research** → **prompts**
2. Open **brainstorm_prompt.md** (right-click → Open with →
   Text editor, or download and open with any text editor)
3. Copy all the text inside (Ctrl+A, Ctrl+C)
4. **Claude users:** go to claude.ai → create a Project called
   "Research Brainstorm" → paste as the project's system instructions
5. **ChatGPT users:** create a Custom GPT with this as instructions
6. **Other AI:** paste as your first message each time
7. Repeat steps 2-6 with **creator_prompt.md** → name this one
   "Aegis Pipeline"
...
When you're ready to share your work, run `publication_check()`
from the Pipeline — it verifies everything was done properly.
For extra rigor, paste your scripts into a third AI conversation
using `prompts/auditor_prompt.md` for independent code review.
...
The Pipeline AI runs an 8-point self-audit on every script before
showing it to you. For publication-quality work, a separate Auditor
AI reviews the code independently — catching bugs that self-checks
miss.

### Keeps you honest about results
Cell 3 prints your pre-registered predictions right next to the
actual results. The predictions were locked before the experiment
ran (SHA-256 verified), so you can't change your hypothesis after
seeing what happened. This blind comparison is generated by code,
not by AI — it can't be biased.
