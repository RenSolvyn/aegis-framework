...
Statistical assumptions are tested automatically. A code-enforced
reality constitution catches impossible values — correlations
above 1, negative counts, effect sizes that violate known limits.
Budget warnings fire at 75% and 90%.

**Shows you the truth.** The last output opens with a clear
verdict — trustworthy or not — computed entirely by code. Then
your locked predictions next to actual results. The AI cannot
soften, spin, or override the verdict.
...
| Physically implausible result | Flagged before celebration — measurement error more likely than discovery |
...
| `tests/test_aegis.py` | 75 tests covering core functionality |
